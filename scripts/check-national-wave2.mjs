...
import { pages } from "./generate-national-wave2.mjs";
...
if (new Set(pages.map((page) => page.slug)).size !== 100) fail("Wave-two slugs are not unique.");
if (new Set(pages.map((page) => page.key)).size !== 100) fail("Wave-two topics are not unique.");
const priorKeys = new Set(waveOnePages.map((page) => page.key));
const priorTitles = new Set(waveOnePages.map((page) => page.title));
for (const page of pages) {
  if (priorKeys.has(page.key)) fail(`${page.slug}: topic key duplicates batch one.`);
  if (priorTitles.has(page.title)) fail(`${page.slug}: title duplicates batch one.`);
}
...
for (const page of pages) {
  ...
  if (!html.includes('data-national-wave="2"')) fail(`${page.slug}: missing batch-two marker.`);
...
for (const prior of waveOnePages) {
...
if (failures.length) {
  console.error(`National wave-two audit failed (${failures.length}):`);
  for (const failure of failures.slice(0, 100)) console.error(`- ${failure}`);
  process.exit(1);
}
console.log(`National wave-two audit passed: ${pages.length} unique guides across ${coveredRegions.size} regions.`);
