...
import { pages } from "./generate-national-wave8.mjs";

const root = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const baseUrl = "https://rukn-legal-vwptio.cranl.net";
const earlier = [...waveOnePages, ...waveTwoPages, ...waveThreePages, ...wavesFourToSevenPages];
const sitemap = readFileSync(resolve(root, "sitemap-national-w8.xml"), "utf8");
const failures = [];
const warnings = [];
...
if (new Set(pages.map((page) => page.slug)).size !== 100) fail("Batch-eight slugs are not unique.");
if (new Set(pages.map((page) => page.key)).size !== 100) fail("Batch-eight keys are not unique.");
if (new Set(pages.map((page) => page.title)).size !== 100) fail("Batch-eight titles are not unique.");
...
for (const page of pages) {
  ...
  if (!html.includes('data-national-wave="8"')) fail(`${page.slug}: missing batch marker.`);
...
if (failures.length) {
  console.error(`National batch 8 audit failed (${failures.length}):`);
  for (const failure of failures.slice(0, 150)) console.error(`- ${failure}`);
  process.exit(1);
}
console.log(`National batch 8 audit passed: ${pages.length} unique guides across ${coveredRegions.size} regions.`);
