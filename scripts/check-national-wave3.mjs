...
import { pages } from "./generate-national-wave3.mjs";

const root = resolve(dirname(fileURLToPath(import.meta.url)), "..");
const baseUrl = "https://rukn-legal-vwptio.cranl.net";
const sitemap = readFileSync(resolve(root, "sitemap-national-w3.xml"), "utf8");
const failures = [];
const warnings = [];
const visible = new Map();
const seen = { title: new Map(), description: new Map(), h1: new Map(), body: new Map() };
const priorPages = [...waveOnePages, ...waveTwoPages];

function fail(message) { failures.push(message); }
function capture(html, pattern) { return html.match(pattern)?.[1]?.replace(/<[^>]+>/g, " ").replace(/\s+/g, " ").trim() || ""; }
function text(html) { return html.replace(/<script\b[\s\S]*?<\/script>/gi, " ").replace(/<style\b[\s\S]*?<\/style>/gi, " ").replace(/<[^>]+>/g, " ").replace(/&(?:nbsp|amp|quot|apos|#\d+|#x[\da-f]+);/gi, " ").replace(/\s+/g, " ").trim(); }
...
if (new Set(pages.map((page) => page.slug)).size !== 100) fail("Wave-three slugs are not unique.");
if (new Set(pages.map((page) => page.key)).size !== 100) fail("Wave-three topic keys are not unique.");
if (new Set(pages.map((page) => page.title)).size !== 100) fail("Wave-three topic titles are not unique.");
const priorKeys = new Set(priorPages.map((page) => page.key));
const priorTitles = new Set(priorPages.map((page) => page.title));
for (const page of pages) {
  if (priorKeys.has(page.key)) fail(`${page.slug}: topic key duplicates an earlier national batch.`);
  if (priorTitles.has(page.title)) fail(`${page.slug}: title duplicates an earlier national batch.`);
}
...
for (const page of pages) {
  ...
  if (!html.includes('data-national-wave="3"')) fail(`${page.slug}: missing batch-three marker.`);
  ...
  if (wordCount < 700) fail(`${page.slug}: content is too short (${wordCount} words).`);
  if (description.length < 100 || description.length > 180) warnings.push(`${page.slug}: description length is ${description.length}.`);
  const nodes = [];
...
for (const prior of priorPages) {
  const html = readFileSync(resolve(root, prior.slug), "utf8");
...
if (failures.length) {
  console.error(`National wave-three audit failed (${failures.length}):`);
  for (const failure of failures.slice(0, 120)) console.error(`- ${failure}`);
  process.exit(1);
}
console.log(`National wave-three audit passed: ${pages.length} unique guides across ${coveredRegions.size} regions.`);
