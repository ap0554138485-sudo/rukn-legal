...
import { pages as futurePages } from "./generate-national-waves4-7.mjs";
...
if (![4, 5, 6, 7].includes(batch)) fail("Batch argument must be 4, 5, 6, or 7.");
if (pages.length !== 100) fail(`Expected exactly 100 pages for batch ${batch}, found ${pages.length}.`);
...
console.log(`National batch ${batch} audit passed: ${pages.length} unique guides across ${coveredRegions.size} regions.`);
